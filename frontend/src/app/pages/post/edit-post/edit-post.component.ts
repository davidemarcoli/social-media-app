    ...
    lastValueFrom(post$)
      .then(value => {
        if (!value) {
          ...
          // TODO: toast-error('Post not found')
        ...
        if (
          !(
            this.authService.getRoles().includes('ROLE_MODERATOR') ||
            this.oldPost.author.username === this.authService.getUsername()
          )
        ) {
          // this.alertService.error('You are not allowed to edit this post')
          // TODO: toast-error('You are not allowed to edit this post')
          this.router.navigate(['/'])
        }
      })
      .catch(reason => {
  ...
  onSubmit(event: any) {
    ...
    lastValueFrom(post$)
      .then(value => {
        ...
        // TODO: toast-success('Post saved successfully')
